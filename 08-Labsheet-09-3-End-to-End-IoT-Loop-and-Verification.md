...
| :---: | :---: | :---: | :---: | :---: | :---: |
| หมุนซ้ายสุด ($0^\circ$) | 0000 | 0% | 0% | [✓] ตรง [ ] ไม่ตรง | TEST OK |
| หมุนประมาณ $45^\circ$ | 1060 | 25% | 25% | [✓] ตรง [ ] ไม่ตรง | TEST OK |
| กึ่งกลาง ($90^\circ$) | 2045 | 50% | 50% | [✓] ตรง [ ] ไม่ตรง | TEST OK |
| หมุนประมาณ $135^\circ$| 3015 | 75% | 75% | [✓] ตรง [ ] ไม่ตรง | TEST OK |
| หมุนขวาสุด ($180^\circ$)| 4095 | 100% | 100% | [✓] ตรง [ ] ไม่ตรง | TEST OK |
...
```
ตอบ
เกิดจาก พอร์ต Serial UART (115200 bps) มากที่สุด
เหตุผล ส่งข้อมูลได้ช้าที่สุดเพียง 11.52 KB/s เมื่อต้องส่งข้อความสตริงโต้ตอบต่อเนื่อง จะเกิดความล่าช้าสะสมในบัฟเฟอร์ได้ง่าย
```
...
```
ตอบ
1) เพื่อให้ระบบ ทำงานต่อได้แบบ Real-Time และปลอดภัย โดยไม่ต้องรอคำสั่งจาก Server/Cloud เนื่องจากอุปกรณ์ Edge อยู่ใกล้เครื่องจักรที่สุด จึงสามารถควบคุมและตัดสินใจตามเงื่อนไขความปลอดภัยฉุกเฉินได้ทันที
2) ระบบจะเกิดภาวะค้าง หรือหยุดทำงานกลางคัน ทำให้ สูญเสียการควบคุม ซึ่งอาจก่อให้เกิดอุบัติเหตุร้ายแรง เครื่องจักรเสียหาย สินค้าชำรุด
```
...
```
ตอบ
เหตุผล บล็อกฮาร์ดแวร์ ADC2 ถูกใช้งานร่วมกัน (Shared Hardware) กับโมดูล Wi-Fi ทำให้ไม่สามารถอ่านค่าอนาล็อกขณะเปิดใช้ Wi-Fi ได้ 
กลไกภายในของชิป เมื่อ Wi-Fi Driver เริ่มทำงาน ไดรเวอร์จะยึดและควบคุม ADC2 เพื่อใช้วัดสัญญาณ RF และจัดการพลังงานของระบบไร้สายโดยตรง หากโปรแกรมพยายามอ่านค่าจาก ADC2 จะคืนค่าผิดพลาด จึงต้องเลี่ยงไปใช้ ADC1 แทน
```
